...
### React Lazy
- cra로 react를 설치했다면 기본적으로 웹팩을 통한 번들링을 제공합니다.
- 하지만 SPA의 특성상 처음 로드될 때 모든 하나로 번들링 된 모든 스크립트를 가져오기 때문에 앱이 커질수록 번들 사이즈가 커져 초기 로딩이 길어진다는 문제점이 있습니다.
- 이러한 문제를 해결하기 위해 react의 lazy를 이용하여 route 단위로 컴포넌트를 동적 import 하였습니다.
